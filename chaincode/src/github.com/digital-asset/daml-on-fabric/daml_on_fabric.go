...
func (t *DamlOnFabric) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "RawBatchWrite" {

		return t.rawBatchWrite(stub, args, rawArgs)
...
// rawBatchWrite: writes a value to K/V world state
func (t *DamlOnFabric) rawBatchWrite(stub shim.ChaincodeStubInterface, args []string, rawArgs [][]byte) pb.Response {

	var errorMessage = ""

	for i := 0; i < len(rawArgs)-1; i++ {
		tempArgs := make([][]byte, 2)

		tempArgs[0] = rawArgs[i]
		i++
		tempArgs[1] = rawArgs[i]

		response := t.rawWrite(stub, args, tempArgs)
	...
	commitLogtempArgs := make([][]byte, 1)
	commitLogtempArgs[0] = rawArgs[len(rawArgs)-1]
	responseCommit := t.writeCommitLog(stub, args, commitLogtempArgs)

	if responseCommit.Status == 500 {
		return shim.Error(responseCommit.Message)
	}
	logger.Infof("---------------------------------------------------")
	logger.Infof("----------------------ENDED CALL-------------------")
	logger.Infof("---------------------------------------------------")
...
func (t *DamlOnFabric) rawWrite(stub shim.ChaincodeStubInterface, args []string, rawArgs [][]byte) pb.Response {
	...
	//logger.Infof("Writing key %v and value %v in ledger.", rawArgs[0], rawArgs[1])
	logger.Infof("Writing key %v and value %v in ledger.", mappedKey, rawArgs[1])
