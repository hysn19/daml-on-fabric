        ...
        private void addBlock(long height, Block block) {
            ...
            try {
                ...
                for (int i = 0; i < bdataCount; i++) {
                    ...
                    TransactionPackage.Transaction txData = TransactionPackage.Transaction.parseFrom(payload.getData());
                    List<TransactionPackage.TransactionAction> al = txData.getActionsList();
                    tx.Actions = new ArrayList<JsonTransactionAction>();
                    for (TransactionPackage.TransactionAction ta : al) {

                        TransactionPackage.ChaincodeActionPayload tap = TransactionPackage.ChaincodeActionPayload.parseFrom(ta.getPayload());//<<<
                        ProposalPackage.ChaincodeProposalPayload ccpp = ProposalPackage.ChaincodeProposalPayload.parseFrom(tap.getChaincodeProposalPayload());
